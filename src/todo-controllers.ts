...
class TodoListController {
  ...
  showModal() {
    const addItemModal =
      new TodoViews.TodoItemFormModalView().createViewElement(
  ...
  showUndoDeleteModal() {
    document
      ...
      ?.prepend(this.undoDeletionModal.createViewElement());
  }

  showEditModal(todoItem: TodoModels.TodoItem) {
    const editModal =
      new TodoViews.TodoItemEditFormModalView().createViewElement(
        DOMManipulation.getProjectTitles(this.projectListModel),
        todoItem,
...
class ProjectListController {
  ...
  constructor(
    ...
    this.projectListView.setDeleteProjectButtonFunction(
      this.showDeleteProjectModal.bind(this)
    );
    this.projectListView.setEditProjectButtonFunction(
      this.showEditProjectModal.bind(this)
    );
  }
  ...
  showAddProjectModal() {
    const modal = new TodoViews.ProjectModalView(
      this.todoList
    ).createViewElement(
      "Add project",
      "Add",
  ...
  refreshProjectListView() {
    ...
    const timeProjectList = document.querySelector(
      ".time-based-list > .menu-list"
    );
  ...
  editProject(projectName: string, newProjectName: string) {
    const project =
      this.projectListModel.projects[
        this.projectListModel.getIndexOfProjectName(projectName)
      ];
  ...
  showEditProjectModal(projectName: string | undefined) {
    const editProjectModal = new TodoViews.ProjectModalView(
      this.todoList
    ).createViewElement(
      "Edit project name",
      "Save",
  ...
  validAddProjectTitle(newProjectTitle: string): boolean {
    ...
    return !DOMManipulation.getProjectTitles(this.projectListModel).includes(
      newProjectTitle
    );
  }
}
