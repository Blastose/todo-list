...
const projectListController = new TodoControllers.ProjectListController(
  projectList,
  projectListView,
  list
);
...
if (
  Misc.populateProjectListFromLocalStorage("ProjectList", projectList, list) ===
  -1
) {
...
const todoListController = new TodoControllers.TodoListController(
  list,
  listView,
  projectList,
  projectListView,
...
todoListController.setRefreshTodoListViewFunction(
  projectListController.refreshProjectListView.bind(projectListController)
);
todoListController.refreshView();

projectListController.setRefreshTodoListViewFunction(
  todoListController.refreshView.bind(todoListController)
);
