...
class TodoItemView {
  createViewElement(
    ...
    const highlight = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-highlight"
    );
    ...
    const labelCheckbox = DOMManipulation.createElementWithClass(
      "label",
      "todo-item-checkbox"
    );
    ...
    const title = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-title"
    );
    title.textContent = todoItem.title;

    const description = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-description"
    );
    description.textContent = todoItem.description;

    const date = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-date"
    );
    date.textContent = format(todoItem.dueDate, "MM/dd/yyyy");

    const projectTitle = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-project-title"
    );
    projectTitle.textContent = todoItem.project;

    const actions = DOMManipulation.createElementWithClass(
      "div",
      "todo-item-actions"
    );
    const editSvgPath = require("./images/pencil.svg");
    const editButton = DOMManipulation.createElementWithClass(
      "img",
      "todo-item-edit"
    );
    ...
    const deleteButton = DOMManipulation.createElementWithClass(
      "img",
      "todo-item-delete"
    );
...
class TodoListView {
  createViewElement(
    ...
    const items = DOMManipulation.createElementWithClass(
      "div",
      "main-todo-items"
    );
    ...
    const addButton = DOMManipulation.createElementWithClass(
      "button",
      "btn-add"
    );
    ...
    const addButtonHeader = document.querySelector(
    ...
    addButtonHeader?.parentNode?.replaceChild(
      newAddButtonHeader!,
      addButtonHeader
    );
...
class ProjectView {
  createViewElement(
    ...
    const sidebarItem = DOMManipulation.createElementWithClass(
      "li",
      "sidebar-item"
    );
    const projectItemContent = DOMManipulation.createElementWithClass(
      "div",
    ...
    const projectItemCount = DOMManipulation.createElementWithClass(
      "span",
      "project-item-count"
    );
...
class ProjectListView {
  ...
  constructor() {
    ...
    this.showHideProjectListButton = document.querySelector(
      ".project-list-show-hide-button"
    )!;
    this.showHideProjectListButton.addEventListener(
      "click",
      this.toggleProjectListCollapse.bind(this)
    );
    this.active = "Default";
    this.hide = false;
  ...
  setEditProjectButtonFunction(
    editProjectFunction: (projectName: string | undefined) => void
  ) {
...
class ModalView {
  static createViewElement(modalTitle: string): HTMLElement {
    ...
    const modalContainer = DOMManipulation.createElementWithClass(
      "div",
      "modal-container"
    );
    modal.appendChild(modalContainer);

    const modalContent = DOMManipulation.createElementWithClass(
      "div",
      "modal-content"
    );
    ...
    const closeButton = DOMManipulation.createElementWithClass(
      "button",
      "close-modal"
    );
...
class ProjectModalView {
  ...
  createViewElement(
    ...
    const form = DOMManipulation.createElementWithClass(
      "form",
      "project-form"
    ) as HTMLFormElement;
    modalContent.appendChild(form);

    const [projectTitleLabel, projectTitleInput] =
      DOMManipulation.createFormInput(
        "text",
        "project-title",
        "form-field",
        "Name"
    ...
    submitButton.addEventListener("click", () => {
      if (validateFunction(projectTitleInput.value)) {
        addFunction(
          new TodoModels.Project(projectTitleInput.value, this.todoList)
        );
        form.reset();
      } else {
    ...
    const errorMessage = DOMManipulation.createElementWithClass(
      "span",
      "error-message"
    );
    errorMessage.classList.add("hide");
...
class TodoItemFormModalBase {
  static createViewElement(
    ...
    const [todoItemTitleLabel, todoItemTitleInput] =
      DOMManipulation.createFormInput("text", "todo-item-title", "", "Name");
    const [todoItemDescriptionLabel, todoItemDescriptionInput] =
      DOMManipulation.createFormInput(
        "text",
        "todo-item-description",
        "",
    ...
    const [todoItemDateLabel, todoItemDateInput] =
      DOMManipulation.createFormInput("date", "todo-item-date", "", "Due date");

    const formView = new FormView([
      new TodoModels.FormItem(todoItemTitleInput, todoItemTitleLabel),
      new TodoModels.FormItem(
    ...
    const [selectPrioLabel, selectPrioOptions] =
      DOMManipulation.selectInputMaker(
        "Priority",
        "priority-list",
        "priority-list",
        // Object values of enums include the number values as well so we need to filter them out
        Object.values(TodoModels.Priority).filter(
...
class TodoItemFormModalView {
  createViewElement(
    ...
    addFunction: (todoItem: TodoModels.TodoItem) => void
  ): HTMLElement {
    const [modal, form, [, , , selectOptions, , submitButton]] =
    ...
    submitButton.addEventListener("click", () => {
      const todoItem = DOMManipulation.extractTodoItemFormValues(
...
class TodoItemEditFormModalView {
  createViewElement(
    ...
    DOMManipulation.selectOption(
      selectPrioOptions,
      TodoModels.Priority[todoItem.priority]
    );

    submitButton.setAttribute("value", "Save");
    submitButton.addEventListener("click", () => {
      const newTodoItem = DOMManipulation.extractTodoItemFormValues(
        form as HTMLFormElement
      );
...
class UndoView {
  ...
  createViewElement() {
    const view = DOMManipulation.createElementWithClass(
      "div",
      "undo-container"
    );
    ...
    const undoButton = DOMManipulation.createElementWithClass(
      "button",
      "btn-undo-delete"
    );
...
class ConfirmModal {
  createViewElement(
    ...
    todoList: TodoModels.TodoList,
    deleteFunction: (projectName: string) => void
  ) {
    const modal = ModalView.createViewElement("Delete project?");
    const modalContent = modal.querySelector(".modal-content")!;

    const warningText = DOMManipulation.createElementWithClass(
      "span",
      "warning-text"
    );
    if (projectName) {
      warningText.innerText = `There are ${todoList.count(
        projectName
      )} todo tasks in ${projectName}. \nDeleting the project will remove all of the todo tasks. \nDo you still want to delete the project?`;
    }
    ...
    const cancelButton = DOMManipulation.createElementWithClass(
      "button",
      "confirm-cancel-button"
    );
    cancelButton.textContent = "Cancel";

    const buttonsDiv = DOMManipulation.createElementWithClass(
      "div",
      "button-container"
    );
...
export {
  ...
  ProjectListView,
  ProjectModalView,
  TodoItemFormModalView,
  TodoItemEditFormModalView,
  UndoView,
  ConfirmModal,
};
